...
TemplateDemo = {{
  ...
  @private TDate = {{
    ...
    export(~{date_format}, _child) = match Date.try_generate_printer(date_format) with
      | { ok=date_printer } -> <>{Date.to_formatted_string(date_printer, Date.now())}</>
      | { ~ko } -> <>Incorrect date format {ko}</>
  }}
  ...
  @private parse(~{ns tag args children }:Template.import_arg(TemplateDemo.tags('a), 'b)) : okko(Template.content(either(TemplateDemo.tags('a), 'b)), Template.failure) =
    if ns == namespace then
      build = match tag
        ...
        | _             -> none
      Option.switch((build -> {ok = Template.to_extension(build(~{args children}))}),
                    {ko = {unsupported_tag ~ns ~tag}},
                    build)
    else {ko = {namespace_not_supported_by_engine =
                "Engine({namespace}) vs Namespace({ns})"}}

  @private export(content, child) =
    e = Template.from_extension(content)
    match e
    | {none} -> {ko = {unknown_tag="Expected extension"}}
    | {some = e} ->
      {ok = match e
