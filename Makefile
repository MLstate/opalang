...
.PHONY: $(BUILD_TOOLS_DIR)/bin/opa
$(BUILD_TOOLS_DIR)/bin/opa: $(MYOCAMLBUILD)
	$(OCAMLBUILD) plugins.qmljs.stamp opa-node-packages.stamp $(target-tool-opa-bin)
	@$(copy-tool-opa-bin)
	@$(OPALANG_DIR)/tools/utils/install.sh --quiet --dir $(realpath $(BUILD_TOOLS_DIR)) --ocaml-prefix $(OCAMLLIB)/../../..

.PHONY: opa
opa: $(BUILD_TOOLS_DIR)/bin/opa
...
opa-create: $(MYOCAMLBUILD)
	...
	$(INSTALL) $(BUILD_TOOLS_DIR)/$(target-tool-opa-create) $(BUILD_TOOLS_DIR)/bin/opa-create
	@chmod 755 $(BUILD_TOOLS_DIR)/bin/opa-create

.PHONY: install-opa-create
install-opa-create:
	@mkdir -p $(PREFIX)/bin
	$(INSTALL) $(BUILD_TOOLS_DIR)/bin/opa-create $(INSTALL_DIR)/bin/opa-create
...
BUILD_NODE_DIR=$(BUILD_TOOLS_DIR)/$(NODE_STDLIB_SUFFIX_DIR)
...
@$(INSTALL) $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/*.bypass "$(STDLIB_DIR)/$*.opp/";
@$(if $(wildcard $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/*NodeJsPackage.js), $(INSTALL) $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/*NodeJsPackage.js "$(STDLIB_DIR)/$*.opp/")
@$(if $(wildcard $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/package.json), $(INSTALL) $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/package.json "$(STDLIB_DIR)/$*.opp/")
endef
...
install-node-pluginopt-%:
	$(if $(wildcard $(BUILD_TOOLS_DIR)/$(PLUGINS_DIR)/$*.opp/),$(install-node-plugin))
...
install-bin:
	...
	@$(if $(wildcard $(BUILD_TOOLS_DIR)/bin/*),$(INSTALL) -r $(BUILD_TOOLS_DIR)/bin/* $(INSTALL_DIR)/bin)
...
install-lib:
	...
	@rm -f $(BUILD_TOOLS_DIR)/lib/opa/static/opabslMLRuntime.cmi
	@mkdir -p $(INSTALL_DIR)/lib/opa
	@$(if $(wildcard $(BUILD_TOOLS_DIR)/lib/opa/*),$(INSTALL) -r $(BUILD_TOOLS_DIR)/lib/opa/* $(INSTALL_DIR)/lib/opa/)
...
install-share:
	...
	@$(if $(wildcard $(BUILD_TOOLS_DIR)/share/opa/*),$(INSTALL) -r $(BUILD_TOOLS_DIR)/share/opa/* $(INSTALL_DIR)/share/opa/)
	@printf "Installation to $(INSTALL_DIR)/share/opa done.[K\n"

install-man:
	@printf "Installing into $(INSTALL_DIR)/share/man[K\r"
	@if [ -d $(BUILD_TOOLS_DIR)/man/man1 ]; then \
	  mkdir -p $(INSTALL_DIR)/share/man/man1; \
	fi
	@$(if $(wildcard $(BUILD_TOOLS_DIR)/man/man1/*.1.gz),$(INSTALL) -r $(BUILD_TOOLS_DIR)/man/man1/*.1.gz $(INSTALL_DIR)/share/man/man1)
...
# 	rm -rf $(INSTALL_DIR)/share/man/man1/opa*
	...
	$(foreach file,$(wildcard $(BUILD_TOOLS_DIR)/bin/*),rm -f $(INSTALL_DIR)/bin/$(notdir $(file));)
