  ...
  let rec aux i =
    (function
     ...
     | c ->
         ...
         assert false)
  and auxn e i = make_cons (wrap_opa_rpc_bson_bson e) (aux i (Bson.IteratorSS.next i))
  in
  wrap_opa_rpc_bson_bson (aux i (Bson.IteratorSS.next i))
...
let insert m f ns (bson:'a) =
  let (bson:opa_rpc_bson_bson) = Obj.magic bson in
...
let update m flags ns selector update =
  let (selector:opa_rpc_bson_bson) = Obj.magic selector in
  let (update:opa_rpc_bson_bson) = Obj.magic update in
...
let query m flags ns numberToSkip numberToReturn query returnFieldSelector_opt =
  let (query:opa_rpc_bson_bson) = Obj.magic query in
  let (returnFieldSelector_opt:opa_rpc_bson_bson option) = Obj.magic returnFieldSelector_opt in
...
##register reply_document : reply, int -> option(RPC.Bson.bson)
