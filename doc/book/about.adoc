...
features of Opa, and as you will see, they are many. To quote but a few, Opa is
...
this impedence mismatch, developers need to add to their application logics
...
With Opa, we set out to provide another, better, way of developing cloud
applications. Opa is designed to get you to your finished app faster,
concentrating only on the interesting parts, without the hassle of writing the
glue or of using a programming language that against its original design and
...
Opa is one technology, consisting in a programming language and its runtime,
...
or work documents. Developers need to combine complex information sources,
extract the results quickly and get them back to the user real-time. As the
...
a bit of advance and show you a simple application: Hello, chat.
...
This command produces a stand-alone executable executable binary, containing
...
required to connect them or to handle malicious connexions or inputs. Opa wrote
...
uses of Opa. Each chapter concentrates on writing one specific application, and
on how best to achieve this using combinations of skills that you have developed
in previous chapters and new skills introduced in this chapter. After this,
additional reference chapters recapitulate in detail all the concepts of the
...
- through the message board of http://opalang.org/downloads[our website];
...
- through http://www.facebook.com/pages/Opalang/196987260339043[Facebook], as Opalang;
