...
This chapter shows an example of binding between <<hello_binding, Opa, OCaml and Javascript>>,
as well as an example to see <<hello_binding_c, C primitives calls from Opa>>.

A complete example of binding with an external Javascript library is available in the chapter
<<hello_recaptcha, hello recaptcha>>.

In Opa, a function written in an other language is called a
<<foreign_function_interface, _foreign function_>>, a _bypass_,
...
- provide a new functionality not available in the standard library, and the implementation cannot
easily be written in Opa (e.g. using low level system call).
Although it does not often happen because of the completeness of the Opa standard library, it remains useful for specific applications.
...
(This chapter contains <<hello_binding_c, an example>> using a external primitive implemented in _C_,
...
In Javascript, we use a special syntax for introducing function names (_##args_),
used for simulating namespaces. Using the directive _##args_ following a _##register_
directive defining the function _stammer_ in the file _plugin_ will introduce
the function named _plugin_stammer_. The use of _##args_ is optional, we can write instead:

.plugin.ml
[source,caml]
-------------------------
include::sources/plugin.ml[]
-------------------------

.plugin.js
[source, C]
-------------------------
include::sources/plugin.js[]
-------------------------
...
Using the directive _##args_ implies a check of arity with the registered type.

Plugin Compilation
^^^^^^^^^^^^^^^^^^^

The plugin files are compiled using the tool <<opa_plugin_builder, _opa-plugin-builder_>>,
distributed with the _Opa_ compiler. You can refer to the documentation of the tool
for more details, and the part about <<filenames_opp, _opp files_>>.
_opa-plugin-builder_ produces an _opp_ directory from plugin sources files.
The name of the plugin should be specified with the option _-o_ :
...
The plugin _plugin.opp_ is ready to be linked with an Opa application.
You can browse the plugin, using the tool <<opa_plugin_browser, _opa-plugin-browser_>>
...
about the external primitives (try _ls -v_).
...
In this example, the key of the function _stammer_ is _plugin_stammer_.
...
their are transformed for computing the key : "-" is replaced by "\_dash\_" and "." is replaced by "\_dot\_" .
...
The <<foreign_function_interface, syntax>> for introducing external primitives in Opa
is _%% key %%_, referring to the _key_ of the external primitive.
...
side implementation of the function _stammer_, with an input for giving the argument.


.hello_bindings.opa
[source,opa]
-------------------------------
include::sources/hello_bindings.opa[]
-------------------------------
...
.Makefile
--------------------
include::sources/c_binding/Makefile[]
--------------------

.freq.c
[source, C]
--------------------
include::sources/c_binding/freq.c[]
--------------------

.c_binding.ml
[source,caml]
--------------------
include::sources/c_binding/c_binding.ml[]
--------------------

.freq.opa
[source,opa]
--------------------
include::sources/c_binding/freq.opa[]
--------------------
...
This paragraph describes the exhaustive list of directives handled by _opa-plubin-builder_ in OCaml and Javascript files.
The <<grammar_conventions, conventions adopted>> to describe the syntax of theses directives are the same as the one used
...
manually, via the <<server_lib, ServerLib>> on the server side, and the <<client_lib, ClientLib>> on the server side.
...
or for attaching extra information to them. The directive _##property_ is used for
...
Syntax of arguments for <<bsl_dir_extern_type, ##extern-type>>
[source, grammar]
--------------------------------------
...
.Syntax of arguments for <<bsl_dir_opa_type, ##opa-type>>
[source, grammar]
--------------------------------------
arguments(##opa-type) ::=
...
[width="70%",cols="3",frame="topbot",options="header"]
|========================================================
...
is to manipulated OCaml values using a special set of macros and function (_CAMLparam_, _CAMLreturn_, etc.), and
...
Now, we will save the code into the file _int.ml_, try to register this function and building an Opa plugin.
...
you want to use it in Opa, so you will need to projects values. To avoid this, _opa_plugin_builder_ transforms
...
projections are not be generated.
====================================================
...
[width="70%",cols="4",frame="topbot",options="header"]
|========================================================
...
An API is available for manipulating opa values, on the <<server_lib, server side>>
as well as on the <<client_lib, client side>>.
...
This example shows how to use the module _Big_int_ of the _OCaml_ standard library to add
...
[source, bash]
--------------------
include::sources/external_types/Makefile[]
--------------------

bigint.ml
[source, caml]
--------------------
include::sources/external_types/bigint.ml[]
--------------------

bigint.opa
[source, opa]
--------------------
include::sources/external_types/bigint.opa[]
--------------------
...
This example shows how to use the opa-type construction from OCaml using the <<server_lib, _ServerLib_>>.
...
[source, bash]
--------------------
include::sources/opa_types/Makefile[]
--------------------

dbm.ml
[source, caml]
--------------------
include::sources/opa_types/dbm.ml[]
--------------------

dbm.opa
[source, opa]
--------------------
include::sources/opa_types/dbm.opa[]
--------------------
