...
get_page(topic) =
  ...
  match MongoCollection.find_one(MongoCollection.orderby(wiki_collection,orderby),pageselect({~topic})) with
  | {success=page} -> page
  | {failure={NotFound}} ->
     default
  | {~failure} ->
 ...
 **/
get_content(topic) = get_page(topic).content
...
@publish save_source(topic, source) =
  page = {topic=topic; _rev=(get_page(topic)._rev)+1; content=source}
  result = MongoCollection.insert_result(wiki_collection,page)
